...
export default {
  ...
  async post(request){
    ...
    const tos = validate(request.payload.tos, 'string', {equal: 'true'});
    debug(formatWithOptions({colors: true}, '[USERS][POST] Validation: %O', {name, email, street_address, password: hash(password), tos}));

    if(!name || !email || !street_address || !password || !tos){
      const invalid = invalidFields({name, email, street_address, password, tos}).join(', ');
