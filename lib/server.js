...
// Routes
server.router = {
  '': handlers.gui.index, // don't confuse method with the filename
  login: handlers.gui.login,
  'sign-up': handlers.gui.signup, 
  api : {
...
server.init = () => {
  ...
  server.httpServer.listen(process.env.PORT || config.httpPort, () => {
    console.log('\x1b[33m%s\x1b[0m', `The server is listening on port ${process.env.PORT || config.httpPort}.`);
  });
  
  

  /* Start the HTTPS server
  server.httpsServer.listen(process.env.PORT || config.httpsPort, () => {
