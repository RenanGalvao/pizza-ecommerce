...
export default async (request, next) => {
  ...
  // If only refresh token is valid, then renew tokens
    ...
    const [accessToken, refreshToken] = await createTokens(tokenData, tokenIdLength, accessTokenMaxAge, refreshTokenMaxAge);
    ...
    debug(formatWithOptions({colors: true}, '[MIDDLEWARES][LOAD_TOKEN] New Request: %O', refreshToken));
    next(); return;
  }
};